...
while (room < 4){
 ...
 if (key == 'D'){
    ...
    Serial.println("Cleared string is:");
...
/*
  Serial.println("Maximum room limit reached");      
  Serial.println("Rooms entered are");
  for(int k = 0; k < 4; k++)
{
  Serial.println(room_nums[k]);
}
*/
    if (digitalRead(15) == LOW && digitalRead(17) == LOW){ //FORWARD
      ...
      node +=1;
      //Serial.println("The bed number is: ");
      //Serial.println(node);
      str_node = String(node);
      //Serial.println("Converted to string");
      //Serial.println(str_node);
      for (j = 0; j<5;j ++){
        if (room_nums[j] == str_node){
        deliver = 1; 
        //Serial.println("Condition satisfied and stopping. Deliver value is: ");
        //Serial.println(deliver);
        }
        else{
        deliver = 0;
        //Serial.println("Condition not satisfied and not stopping. Deliver value is: ");
        //Serial.println(deliver);
      }
